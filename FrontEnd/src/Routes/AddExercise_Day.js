...
const AddExercise_Day = () => {
    ...
    function eliminar_dia(elemento,idEjercicio) {
        ...
        axios.delete("http://localhost:7777/api/exerciseDay/" + idEjercicio, {idExercise : idEjercicio}).then((response) => {
    ...
    const crearBotonEjercicios = (ejercicio) => {
        ...
        let TipoEjercicio = todosLosEjs.find((ej) => ej.id === ejercicio.exerciseId);

        console.log(TipoEjercicio)
        
        const liEjercicio = document.createElement("li");
        liEjercicio.classList.add("d-flex", "justify-content-between", "align-items-center");
    
        const ejercicio_container = document.createElement("div");
        ejercicio_container.className = "alert alert-secondary d-flex align-items-center gap-3 botonDiaLista";        
        ...
        ejercicio_container.appendChild(imagen_container);
        ...
        if(rutinaActual.user_id === user.id){
          ...
          boton_eliminar.addEventListener("click", () => eliminar_dia(liEjercicio, ejercicio.idExercise));
    ...
    const getDia = async () => {
        ...
        const response = await axios.get(
            "http://localhost:7777/api/day/" + idDia
        );
        console.log(response.data.data);
        return response.data.data;
    };
        ...
        <div>
            ...
            {loaded && (
                ...
                <div className="mt-5 card container p-4 justify-content-center align-items-center overflow-auto">
                    ...
                    <h3>Dia: <label style={{ color: '#fb465a' }}>{diaActual.name}</label></h3>

                    {rutinaActual !== null && (
                        <div className="mt-5 card container p-4 align-items-center overflow-auto">
                            <ul className="align-items-center listaScroleableEjs" id="listaEjercicios" style={{ overflowY: 'scroll', padding: '30px', maxHeight: '400px', 'marginBottom': '30px' }}></ul>
                    ...
                    {rutinaActual.user_id === user.id && (
                        <button className="btn btn-danger clickAddEjercicio" onClick={mostrarVentana} >
                            Añadir Ejercicio
                        </button>
                    )}

                    <div className="mt-2">
                        <button className="btn btn-danger clickAddDia"
                            onClick={() => window.location.href = "/rutina/" + rutinaActual.id}>
                            Volver a la rutina</button>
                    </div>
                </div>
